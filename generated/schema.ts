...
export class ZuniswapFactory extends Entity {
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save ZuniswapFactory entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type ZuniswapFactory must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("ZuniswapFactory", id.toString(), this);
    }
  }

  static load(id: string): ZuniswapFactory | null {
    return changetype<ZuniswapFactory | null>(store.get("ZuniswapFactory", id));
  }
...
export class ZuniswapDayData extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save ZuniswapDayData entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type ZuniswapDayData must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("ZuniswapDayData", id.toString(), this);
    }
  }

  static load(id: string): ZuniswapDayData | null {
    return changetype<ZuniswapDayData | null>(store.get("ZuniswapDayData", id));
  }
